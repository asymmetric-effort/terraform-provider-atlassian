// Package customfield implements the atlassian_jira_custom_field read-only data source.
//
// This data source reads Jira custom fields by ID from the Atlassian Cloud REST API.
package customfield
...
// Schema defines the schema for the jira custom field data source.
func (d *DataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Reads a Jira custom field from Atlassian Cloud by ID.",
...
// Read retrieves custom field data from the Atlassian API.
func (d *DataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	err := d.client.Get(ctx, fmt.Sprintf("/rest/api/3/field/%s", identifier), &cf)
	if err != nil {
		if apiErr, ok := err.(*atlassian.APIError); ok && apiErr.StatusCode == http.StatusNotFound {
			resp.Diagnostics.AddError(
				"Custom field not found",
				fmt.Sprintf("Jira custom field %q not found. Verify the ID is correct.", identifier),
			)
			return
		}
		resp.Diagnostics.AddError(
			"Failed to read custom field",
			fmt.Sprintf("Could not read Jira custom field %q: %s", identifier, err.Error()),
		)
		return
	}

	config.ID = types.StringValue(cf.ID)
